 ...
 btnSearch.addEventListener('click',function(){
    ...
    if (valueSearchInput){
        for(let i = 0; i < list.length; i++){
            ...
            let inforSearch = list[i].name.includes(valueSearchInput) || list[i].number.includes(valueSearchInput) ? list[i] : null
            if(inforSearch){
...
btnClear.addEventListener('click',function(){
    ...
    const contact ={};
    information.forEach(cont=>{
        if(!contact[cont.number]){
        contact[cont.number] = cont;
    }
    });
    console.log(contact)
    information = Object.keys(contact).map((key)=>{
        return contact[key]
    });
    localStorage.setItem('information',JSON.stringify(information))
    rederInformation(information)
})
